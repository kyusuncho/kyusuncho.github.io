...
This list includes a brief description of each style change. To revert, we recommend users to override the change with [custom styles]({{site.baseurl}}/blog/customization/#override-and-completely-custom-styles) or [custom variables]({{site.baseurl}}/blog/customization/#override-and-define-new-variables) (rather than replacing theme `_scss` files), which will future-proof their site from future changes.
...
We've added several custom `_includes` to provide users with more customization options for different site elements. We've also added a section to [Configuration]({% link customization.md %}#override-includes) to outline these.
...
For more, see [Custom layouts and includes]({% link customization.md %}#custom-layouts-and-includes). PR: [#1058].
...
In addition to customizing the search placeholder, we've also added the ability to provide custom content to the search index. for more, see [Custom content for search index]({% link search.md %}#custom-content-for-search-index). PR: [#1068].
...
- Links wrap. PR: [#905]
- Callouts: a new component similar to alerts or banners. See [UI Components - Callouts]({% link ui-components/callouts.md %}). PR: [#466]
